...
def main(data_dir, output_dir):
    ...
    with timer("Loading data"):
        ...
        weather_train_df = load_weather_csv(
            data_dir + "/raw/weather_train.csv")
        weather_test_df = load_weather_csv(data_dir + "/raw/weather_test.csv")
        building_df = load_building_csv(
            data_dir + "/raw/building_metadata.csv")
    ...
    with timer("Merging weather and site"):
        weather_train_df = weather_train_df.merge(
            site_df, on="site_id", how="left")
        weather_test_df = weather_test_df.merge(
            site_df, on="site_id", how="left")
    ...
    with timer("Merging main and weather"):
        ...
        test_df = test_df.merge(weather_test_df, on=[
                                "site_id", "timestamp"], how="left")
...
def impute_weather_data(data_frame):
    ...
    weather_imputed["hour"] = pd.Categorical(
        weather_imputed["timestamp"].dt.hour)
    weather_imputed["weekday"] = pd.Categorical(
        weather_imputed["timestamp"].dt.dayofweek)
    weather_imputed["month"] = pd.Categorical(
        weather_imputed["timestamp"].dt.month)
    ...
    weather_final = weather_final.drop(
        columns=["hour", "weekday", "month"], axis=1)
...
def localize_weather_timestamp(df):
    ...
    # Because of DST we can have duplicates here
